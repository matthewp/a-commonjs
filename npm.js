/**
 * NPM overrides for CommonJSLoader
 */
...
//exports.includeInBuild = true;
  ...
  window.NPMLoader = NPMLoader;

  function NPMLoader(){
    CommonJSLoader.apply(this, arguments);
  }

  NPMLoader.prototype = Object.create(CommonJSLoader.prototype);
  NPMLoader.prototype.constructor = NPMLoader;

  var proto = NPMLoader.prototype;

	var oldDepend = proto.depend;
	proto.depend = function(id, deps){
		var loader = this;

		if(loader.npm) {
			return oldDepend.apply(this, arguments);
		}

		return new Promise(function(resolve, reject){

			var address = utils.path.removeJS(loader.locate("package.json"));

			return loader.fetch("package.json", null, address).then(function(source){
				var context = {
					packages: [],
					loader: loader,
					// places we
				...
				var pkg = {origFileUrl: address, fileUrl: address};

				crawl.processPkgSource(context, pkg, source);
				...
				return crawl.deps(context, pkg, true).then(function(){
					...
					// debugger we are done
					translateConfig(loader, packages);

					return oldDepend.call(loader, id, deps).then(function(deps){
						resolve(deps);
					});
				});
			});
...
//exports.addExtension = function(System){
	 ...
	 */
	var oldNormalize = proto.normalize;
	proto.normalize = function(name, parentName, parentAddress){
	...
	var oldLocate = proto.locate;
	proto.locate = function(name){

		var parsedModuleName = utils.moduleName.parse(name),
			loader = this;
		// @ is not the first character
		if(parsedModuleName.version && this.npm /*&& !loader.paths[name]*/) {
			var pkg = this.npm[parsedModuleName.packageName];
			if(pkg) {
				var address = oldLocate.apply(this, arguments);
